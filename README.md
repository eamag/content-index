...
| `filter`            | `(v: ProcessedContent, ctx: BuildCtx) => boolean` | requires a non-empty `relativePath` and `slug` | Controls which pages are indexed at all (sitemap.xml, contentIndex.json, and RSS). Return `false` to exclude a page from every output. |
| `rssFilter`         | `(v: ProcessedContent, ctx: BuildCtx) => boolean` | excludes pages not backed by a real file (e.g. tag/folder pages), based on `ctx.allSlugs` | Controls which already-indexed pages additionally show up in the RSS feed. Pages excluded here still appear in sitemap.xml and contentIndex.json. Use this to keep virtual pages out of "recent notes" without hiding them from the site's sitemap or explorer sidebar. |
